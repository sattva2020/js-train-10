...
function customObjectFromEntries(entries) {
  ...
  // Перетворення числового значення на рядок
    const newMapentries = entries.map(([key, value]) => {
    ...
    // Використання методу `fromEntries` для створення об'єкта з масиву записів
      const resultObject = Object.fromEntries(newMapentries);
...
function extendAndFreezeObject(obj1, obj2) {
  ...
  console.log(`isFrozen obj1: ${Object.isFrozen(obj1)}`);
...
function checkOwnership(obj1, obj2, key) {
  ...
  if (hasKeyInObj1 && hasKeyInObj2) {
    return obj1[key] === obj2[key]; // Якщо значення однакові, повернемо true.  // Якщо відрізняються, повернемо false.
  };
...
function convertArrayToObj(arr) {
  ...
  // Застосовуємо метод Object.fromEntries() для створення об'єкта
    const fromEntriesObj = Object.fromEntries(Object.entries(nullObj));
